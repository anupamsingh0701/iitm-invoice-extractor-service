...
def parse_date_to_iso(text: str) -> Optional[str]:
    ...
    # Pattern 2.2: MM-DD-YYYY or DD-MM-YYYY (4-digit year)
    m2 = re.search(r'\b(0?[1-9]|[12]\d|3[01])[-/.](0?[1-9]|[12]\d|3[01])[-/.](\d{4})\b', text)
    if m2:
        val1, val2, y = int(m2.group(1)), int(m2.group(2)), int(m2.group(3))
        if val1 > 12:
    ...
    if m3:
        val1, val2, y_short = int(m3.group(1)), int(m3.group(2)), int(m3.group(3))
...
def parse_invoice_no(text: str) -> Optional[str]:
    pattern = r'\b(?:invoice\b|inv\b|bill\b|receipt\b|reference\b|ref\b)(?:\s+(?:no\.?|number|#|code|num))?[:\s\-#]*((?!(?:date|due|amount|total|tax|to|for|gst|vat|rs|inr|usd|ref|reference)\b)[a-zA-Z0-9\-_]+)\b'
